...
class Polyline(Segment):
    ...
    def drawHe(self, _t1, _t2, _orient):

        seg_target = Line(self.pts[0], self.pts[1])
        dmax = seg_target.length(0, 1)

        for i in range(1, len(self.pts)-1):
            seg = Line(self.pts[i], self.pts[i+1])
            dist = seg.length(0, 1)
        ...
        offset = 0.04*dmax
        p1, p2 = CompGeom.computeLine_offset(
            seg_target, offset, _t1, _t2, _orient)
        offset_line = Line(p1, p2)

        if _orient:
            tr_a = p2
            tr_b = offset_line.getPoint(0.9)
            aux_line = Line(p2, tr_b)
        else:
            tr_a = p1
            tr_b = offset_line.getPoint(0.1)
            aux_line = Line(p1, tr_b)

        _, tr_c = CompGeom.computeLine_offset(
            aux_line, offset*0.5, 0, 1, False)

        triangle = [tr_a, tr_b, tr_c]

        return offset_line, triangle
