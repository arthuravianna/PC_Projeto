...
class MEKR:
    def __init__(self, segment, v_begin, v_end, v_begin_next, v_end_next, face, edge=None):
    ...
    def execute(self):
        ...
        # get the half-edges
        he1 = HalfEdge.inBetween(self.v_begin, self.v_begin_next, self.face)
        he2 = HalfEdge.inBetween(self.v_end, self.v_end_next, self.face)
...
class KEMR:
    ...
    def unexecute(self):
        mekr = MEKR(None, self.v_begin, self.v_end,
                    self.v_begin_next, self.v_end_next, self.face, self.edge)
        mekr.execute()
